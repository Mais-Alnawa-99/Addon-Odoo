...
class PropertyController(http.Controller):
    ...
    def property_listing(self, **kwargs):
        ...
        return request.render('real_estate_management.property_website_template', {'properties': properties})

    @http.route('/property/<int:property_id>', auth='user', website=True)
    def property_item(self, property_id):
        """ Shows each corresponding property's view in property_view_item """
        property = request.env['property.property'].sudo().browse(property_id)
        return request.render('real_estate_management.property_details_template', {
            'property': property, 
        })
